...
public class RealJob 
{
	...
	public void test() 
	{
		...
		for(int i = 0; i < num1.length; i++)
			Edge.driver.findElement(By.id(Character.toString(num1[i]))).click();
		
		Edge.driver.findElement(By.id(op)).click();
		
		char[] num2 = number2.toCharArray();
		
		for(int i = 0; i < num2.length; i++)
			Edge.driver.findElement(By.id(Character.toString(num2[i]))).click();
		
		Edge.driver.findElement(By.id("ravno")).click();
		
		assertEquals(exp, Edge.driver.findElement(By.id("res")).getAttribute("value"));
	}
}